...
// Note: You'll need to implement authentication management separately
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($prenom) || empty($nom) || empty($password) || empty($confirm_password)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, password, role, nom, prenom) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $username, $hashed_password, $role, $prenom, $nom);
